//
//  MidiQuestionExtensionDSPKernel.hpp
...
#import "MidiQuestionExtension-Swift.h"
#import "MidiQuestionExtensionParameterAddresses.h"

constexpr uint16_t kMaxVelocity = std::numeric_limits<std::uint16_t>::max();

/*
 MidiQuestionExtensionDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class MidiQuestionExtensionDSPKernel {
public:
    ...
    // Add a case for each parameter in MidiQuestionExtensionParameterAddresses.h
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case MidiQuestionExtensionParameterAddress::midiNoteNumber:
                mNextNoteToSend = (uint8_t)value;
                break;
            case MidiQuestionExtensionParameterAddress::sendNote:
    ...
    void handleMIDIEventList(AUEventSampleTime now, AUMIDIEventList const* midiEvent) {
        /*
         ...
         auto thisObject = static_cast<MidiQuestionExtensionDSPKernel *>(context);
